...
from AkkiX import YouTube, app
from AkkiX.core.call import Akki
from AkkiX.misc import SUDOERS, db
from AkkiX.utils import bot_sys_stats
from AkkiX.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from AkkiX.utils.formatters import seconds_to_min
from AkkiX.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from AkkiX.utils.stream.autoclear import auto_clean
from AkkiX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Akki.pause_stream(chat_id)
        ...
        await Akki.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Akki.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Akki.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Akki.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Akki.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
